...
async def is_stop_intent(llm: GeminiClient, message: str) -> bool:
    ...
    if len(message) < 100 and "?" not in message:
        prompt = f'''Определи, хочет ли пользователь ЗАВЕРШИТЬ интервью.
Сообщение: "{message}"
Примеры завершения: "всё", "хватит", "давай отчёт", "устал", "пока"
НЕ завершение: технический ответ, вопрос к интервьюеру
